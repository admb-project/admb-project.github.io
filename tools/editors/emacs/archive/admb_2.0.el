...
(defcustom admb-comp-command "adcomp"
  "Shell command to compile C++ to object code."
  :tag   "Compile"
  :type  'string
  :group 'admb)
(defcustom admb-link-command "adlink"
  "Shell command to link object code to executable."
  :tag   "Link"
  :type  'string
  :group 'admb)
(defcustom admb-make-command "admake"
  "Shell command to build executable from TPL."
  :tag   "Make"
  :type  'string
  :group 'admb)
(defcustom admb-tpl2cpp-command "tpl2cpp"
  "Shell command to translate TPL to C++."
  :tag   "tpl2cpp"
  :type  'string
  :group 'admb)
...
(defun admb-compile () "Compile C++ to object code." (interactive)
  (delete-other-windows)(shell-command (concat admb-comp-command " " (file-name-sans-extension (buffer-name))))
  (set-buffer "*Shell Command Output*")(setq show-trailing-whitespace nil))
...
(defun admb-link () "Link object code to executable." (interactive)
  (delete-other-windows)(shell-command (concat admb-link-command " " (file-name-sans-extension (buffer-name))))
  (set-buffer "*Shell Command Output*")(setq show-trailing-whitespace nil))
(defun admb-make () "Build executable from TPL." (interactive)
  (delete-other-windows)(shell-command (concat admb-make-command " " (file-name-sans-extension (buffer-name))))
  (set-buffer "*Shell Command Output*")(setq show-trailing-whitespace nil))
...
(defun admb-run () "Run ADMB program." (interactive)
  (delete-other-windows)(shell-command (file-name-sans-extension buffer-file-name)))
(defun admb-run-args (args) "Run ADMB program with ARGS." (interactive "sArgs: ")
  (delete-other-windows)(shell-command (concat (file-name-sans-extension buffer-file-name) " " args)))
...
PROCEDURE_SECTION
  ...
  RSS = norm2(Y-Yfit);
")(goto-char (point-min))(delete-char 1))
(defun admb-tpl2cpp () "Translate TPL to C++." (interactive)
  (delete-other-windows)(save-buffer)
  (shell-command (concat admb-tpl2cpp-command " " (file-name-sans-extension (buffer-name)))))
...
(defvar admb-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map [?\C-c ?\C-l] 'admb-clean        )
    (define-key map [?\C-c ?\C-o] 'admb-rep          )
    (define-key map [?\C-c ?\C-t] 'admb-tpl2cpp      )
...
`admb-tpl2cpp-command' accordingly. After compiling,use `admb-cpp' to debug,
`admb-clean' to remove temporary files,`admb-run' or `admb-run-args' to run, and
`admb-rep' to view model output.
