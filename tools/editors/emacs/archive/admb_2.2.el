...
(defun admb-run () "Run ADMB program." (interactive)
  (let ((tpl-buffer (buffer-name)))
    (if (and admb-window-right (one-window-p))(split-window-horizontally))
    (shell-command (file-name-sans-extension buffer-file-name))(select-window (get-buffer-window "*Shell Command Output*"))
    (setq show-trailing-whitespace nil)(goto-char (point-max))(select-window (get-buffer-window tpl-buffer))))
