...
(defun admb-run () "Run ADMB program." (interactive)
  (if (and admb-window-right (one-window-p))(split-window-horizontally))
  (shell-command (file-name-sans-extension buffer-file-name))(end-of-buffer-other-window 0)
...
(defun admb-run-args (args) "Run ADMB program with ARGS." (interactive "sArgs: ")
  (if (and admb-window-right (one-window-p))(split-window-horizontally))
  (shell-command (concat (file-name-sans-extension buffer-file-name) " " args))(end-of-buffer-other-window 0)
  (set-buffer "*Shell Command Output*")(setq show-trailing-whitespace nil))
...
(defun admb-tpl2cpp () "Translate TPL to C++." (interactive)
  (save-buffer)(if (and admb-window-right (one-window-p))(split-window-horizontally))
  (compile (concat admb-tpl2cpp-command " " (file-name-sans-extension (buffer-name))))
  (set-buffer "*compilation*")(setq show-trailing-whitespace nil))
...
(defvar admb-mode-map
  ;; Special   C-c C-        h
  ;; Custom    C-c C- a cd f     l  o    t vw
  ;; Available C-c C-  b  e g ijk mn pqrs u  xyz
