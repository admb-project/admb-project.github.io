...
(defun admb-run () "Run ADMB program." (interactive)
  (let ((tpl-buffer (buffer-name))(resize resize-mini-windows))
    (if (and admb-window-right (one-window-p))(split-window-horizontally))(setq resize-mini-windows nil)
    (shell-command (file-name-sans-extension buffer-file-name))(setq resize-mini-windows resize)
    (select-window (get-buffer-window "*Shell Command Output*"))(setq show-trailing-whitespace nil)(goto-char (point-max))
    (select-window (get-buffer-window tpl-buffer))))
(defun admb-run-args (args) "Run ADMB program with ARGS." (interactive "sArgs: ")
  (let ((tpl-buffer (buffer-name))(resize resize-mini-windows))
    (if (and admb-window-right (one-window-p))(split-window-horizontally))(setq resize-mini-windows nil)
    (shell-command (concat (file-name-sans-extension buffer-file-name) " " args))(setq resize-mini-windows resize)
    (select-window (get-buffer-window "*Shell Command Output*"))(setq show-trailing-whitespace nil)(goto-char (point-max))
    (select-window (get-buffer-window tpl-buffer))))
